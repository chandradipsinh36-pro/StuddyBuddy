  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const currentCatName = (courseData.category as any)?.name || cats.find(c => c.categoryId === courseData.categoryId)?.name || '';
        setSubjectCategory(currentCatName || 'Mathematics');
        ...
        // Only keep resources belonging to the current logged-in tutor
        const fetchedResources = (myResourcesRes.data || []).filter(r => {
        ...
        // For existing attached resources, only include if owned by this tutor
